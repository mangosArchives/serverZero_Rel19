...
* first part is the order_number, revision and counter that shows the commit
  ...
  sets global order in directory for backported SQL updates (it's backported in
...
* Add the update to commit and manually include sql update changes in the related
  database file.
* Now use `git_id -s`. The update revison will updates in name, guards will added
